 ...
 *
 * 세줄요약
 * A가 [1,2,3,4] 이고 K가 2이면 [3,4,1,2]를 반환
 * K가 1이면 [4,1,2,3]을 반환
 * K번 만큼 오른쪽으로 밀면됨
 * */
...
/**
 * 다른 사람들의 풀이 방식 응용해본 풀이
 * */
function solution1(A, K){
    const rotationNum = K > A.length ? K%A.length : K;
    const cutLineNum = A.length-rotationNum;
    return [ ...A.slice(cutLineNum), ...A.splice(0,cutLineNum) ];
}

console.log(solution1(list, count));